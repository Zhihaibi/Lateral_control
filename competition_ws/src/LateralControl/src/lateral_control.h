...
protected:
    // 下列参数是车辆的物理参数
    double ts_ = 0.01;  // 控制周期
    double cf_ = 155494.663;  // 前轮侧偏刚度，左右轮之和
    double cr_ = 155494.663;  // 后轮侧偏刚度，左右轮之和
    double wheelbase_ = 2.7;  // 前后轴轴距
    double mass_ = 1400.0;  // 车辆质量
    double lf_ = 1.0;  // 前轴中心到质心距离
    double lr_ = 1.2;  // 后轴中心到质心距离
    double iz_ = 1400.0;  // 车辆绕z轴的转动惯量
    double steer_ratio_ = 16.8;  // 方向盘转角和前轮转角之比
    double steer_single_direction_max_degree_ = 10.0;  // 方向盘单边的最大转角 rad
    double max_lat_acc_ = 5.0;  // 最大允许横向加速度
    double minimum_speed_ = 0.1;  // minimum speed protection
...
public:
    ...
    void LqrTableInitial();

    void LqrLookUpTable(double vx);
